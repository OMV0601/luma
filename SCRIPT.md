...
- [ ] Run the auto-demo once end-to-end: Welcome → **▶ Watch the auto-demo** (~3 min hands-free — it plays a real round AND tours voice mode, the jargon translator, the Codex, and accessibility). It must be green — it's your nuclear fallback (below) and the judges' leave-behind. (There's also a **Take a guided tour** button — same drive, but the viewer clicks through at their own pace.)
...
**[SCREEN]** `/for-institutions`, then the end card (QR + live URL). *(The landing page behind that QR opens with **▶ Watch the auto-demo** — a fully hands-free run that plays a real round and tours every feature with spotlights and popup cards. After this video ends, the app is its own salesperson.)*
...
- [ ] Same rig, same roles for live Q&A day — plus `QA_PREP.md` re-read out loud by all three, and two things pre-loaded to hand the judges: `/play?scenario=payday` (the chair, for a judge who wants to play) and the Welcome page's **Take a guided tour** (for a judge who'd rather click through at their own pace). Use **▶ Watch the auto-demo** if they'd rather just watch.
...
**Nuclear option:** if the DRIVER can't drive at all (internet, hardware, nerves), open the Welcome page and click **▶ Watch the auto-demo**. The app takes the cursor and runs the entire demo itself, fully hands-free — types the probe question into a real round, flags the tactic, takes the deal, opens the Evidence File, tours the Codex and accessibility, answers a voice-mode scam call and hangs up, opens the lease jargon-translator, ends on the business page. It auto-advances on a timer with a visible progress bar; a teammate narrates over the top and can hit **Pause** (or Space) anytime to hold a card longer. If you'd rather control the pace by hand, use **Take a guided tour** instead — same drive, Next-button paced. The demo literally cannot be fumbled: the product does the clicking.
